...
| E3 | WRONG | MED | `rules.md` L78; `tech-stack.md` L163 | Never parallelize requests to the same Letterboxd account; per-domain semaphore + ≥2 s spacing | Rate limit is per-`LetterboxdClient` INSTANCE (`services/letterboxd/client.py:46-83`); each scheduler/onboarding job builds its own client (`scheduler.py:260,279,310`; `services/onboarding.py:107,182`), so concurrent jobs DO hit Letterboxd in parallel; no global semaphore exists | `grep -n "LetterboxdClient(settings)" src/watchlistarr/scheduler.py src/watchlistarr/services/onboarding.py` |
...
| E9 | STALE | LOW | `tech-stack.md` L241; `architecture.md` L91 | `DATABASE_URL=sqlite+aiosqlite:///data/watchlistarr.db` (3 slashes, relative) | Container env is 4 slashes, absolute: `Dockerfile:15` `sqlite+aiosqlite:////data/watchlistarr.db`; `.env.example:6` concurs; 3-slash form is only the bare `config.py:45` local default | `grep -n "DATABASE_URL" Dockerfile .env.example src/watchlistarr/config.py` |
...
| E19 | WRONG | HIGH | `sync-strategy.md` L121-130 | After add-user only discovery + films-backstop run; watchlist NOT auto-scraped; full sync happens per list on activation | `_initial_run` (`services/onboarding.py:100-146`) does ensure-watchlist-row → discovery → films-backstop → FULL SYNC of EVERY discovered list, watchlist included, while all stay `enabled=False`; toggle-on additionally kicks an immediate full sync (`routes/api/v1.py:578-591`); big scraping-cost implication | `sed -n 100,146p src/watchlistarr/services/onboarding.py` |
...
| E21 | STALE | MED | `ui-features.md` L43 | Add-user launches `_initial_run_in_background` (ensure + discovery + backstop) | Function is `schedule_initial_run` → `_initial_run` (`services/onboarding.py:149-157,100`) and the run ALSO full-syncs every discovered list (see E19) | `grep -n "_initial_run\|schedule_initial_run" src/watchlistarr/services/onboarding.py src/watchlistarr/routes/api/v1.py` |
...
| E36 | WRONG | LOW | `versioning.md` L11 | MAJOR examples include endpoint "`/radarr/list/{id}` y la forma del JSON" | No `/radarr/...` route was ever registered; the stable surfaces are `/{user}/{slug}/`, `/{user}/watchlist/`, `/lists/{slug}/` (`routes/api/radarr.py`) | `grep -rn "radarr/list" src/` (expect no matches) |
...
| E43 | STALE | LOW | `data-model.md` L102 | "`resolve_film` re-resuelve... cuando `imdb_id` ya está, devuelve la fila cacheada sin tocar HTTP" | Function is `resolve_films` (batch, since v1.0.2); cache hit requires BOTH `imdb_id` AND `letterboxd_avg_rating` non-NULL (`services/scrape/film_resolver.py:118-124`) — rating-NULL rows re-fetch | `sed -n 112,126p src/watchlistarr/services/scrape/film_resolver.py` |
...
Write: "doc X currently claims Y — wrong as of 2026-07, see E*n* in `watchlistarr-docs-and-writing`". Never copy the row's content into another skill or doc.
...
- README voice anchor (v1.5.2 end-user rewrite): `grep -n "README rewritten" CHANGELOG.md`. Commit sha `9ca0f8a` per project history notes (sha itself unverified from the working tree; the rewrite is verified via the CHANGELOG entry).
