...
**Create a migration** (models changed first, DB at current head):

```bash
uv run alembic revision --autogenerate -m "short description"
```

House convention (verified across all 9 files): revision ids are sequential zero-padded strings,
not hashes. After autogenerate, rename the file to `NNNN_snake_name.py` and set
`revision = "NNNN"` / `down_revision = "<NNNN-1>"` to continue the chain. Review the
autogenerated ops — SQLite partial indexes (`sqlite_where=`) and data migrations are always
hand-written here.
...
`tech-stack.md` §Docker shows an **older draft**; trust `Dockerfile` (26 lines). Two stages,
...
- `HTTP_PORT` moves ONLY the host side of the mapping; a fresh clone defaults to **8080**.
  The `:8088` sprinkled through CLAUDE.md/workflows.md exists only because the owner's
  uncommitted `.env` sets `HTTP_PORT=8088` — on a fresh clone `curl :8088/healthz` fails; use
  8080 or set `HTTP_PORT` yourself. Day-to-day QC loop lives in `watchlistarr-run-and-operate`.
...
| `.env.example` traps (4-slash URL, UA pin) | `grep -n "DATABASE_URL\|USER_AGENT" .env.example src/watchlistarr/config.py` |
